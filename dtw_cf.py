...
def cf_augment(df, initializer = "random", mode = 1, draw_example = False, draw_pca = False, draw_tsne = False) :
    """
    ...
    for label_to_augment, count_label in label_count.items() :#Iterate over each label to make it have the same count as the label with the maximum count
        ...
        if label_count[label_to_augment] < label_count[label_max] :
            ...
            if draw_pca == True:
                #Draw PCA of all the data, with the new samples in red
                
                pca = PCA(n_components=2)
                data = df_old.drop([0], axis=1).to_numpy(dtype = np.double)
                X = df.drop([0], axis=1).to_numpy(dtype = np.double)
                
                pca.fit(data)
                data = pca.transform(data)
                X = pca.transform(X)
                # Create 2 subplot with and without the new samples
                fig, ax = plt.subplots(1, 2, figsize=(30, 10), dpi = 200)
                ## Without new samples
                
                ax[0].scatter(data[:,0], data[:,1], c=df_old[0].to_numpy(dtype = np.double), cmap = "tab10")
                ax[0].set_title("PCA")
                
                ax[1].scatter(X[:,0], X[:,1], c=df[0].to_numpy(dtype = np.double), cmap = "tab10")
                ax[1].scatter(X[df.shape[0] - new_sample.shape[0]:,0], X[df.shape[0] - new_sample.shape[0]:,1], c="red")
                plt.savefig(f'Plots/PCA/PCA_mode{mode}_initializer{initializer}.png')
                plt.show()
                plt.close()
            ...
            if draw_tsne == True:
                ...
                ax[1].scatter(X[:,0], X[:,1], c=df[0].to_numpy(dtype = np.double), cmap = "tab10")
                ax[1].scatter(X[df.shape[0] - new_sample.shape[0]:,0], X[df.shape[0] - new_sample.shape[0]:,1], c="red")
                plt.savefig(f'Plots/TSNE/TSNE_mode{mode}_initializer{initializer}.png')
                plt.show()
                plt.close()
